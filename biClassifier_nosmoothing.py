...
def predict(tokens):
    logp_pos = 0
    logp_neg = 0
    if tokens[0] in dic_unigram and tokens[0] in dic_starting:
        if dic_starting[tokens[0]].count_pos > 0:
            logp_pos += log(dic_starting[tokens[0]].count_pos/starting_pos)
        if dic_starting[tokens[0]].count_neg > 0:
            logp_neg += log(dic_starting[tokens[0]].count_neg/starting_neg)
    for idx, token in enumerate(tokens[0:-1]):
        if (token, tokens[idx + 1]) not in dic_bigram:
            continue
        if dic_bigram[(token, tokens[idx + 1])].count_pos > 0:
            logp_pos += log(dic_bigram[(token, tokens[idx + 1])].count_pos/dic_unigram[token].count_pos)
        if dic_bigram[(token, tokens[idx + 1])].count_neg > 0:
            logp_neg += log(dic_bigram[(token, tokens[idx + 1])].count_neg/dic_unigram[token].count_neg)
...
output = open(f"./prediction/Bigram_nosmoothing.txt", "w+")
